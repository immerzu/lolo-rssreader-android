param(
    ...
    [string]$OutputDir = "F:\001_Coding_Projekte\RSS_Reader_Android\Ausgabe_APK",
...
$crashLogPath = Join-Path $OutputDir "rss-reader-webview-crash-log-$timeStamp.txt"
$debugLogPath = Join-Path $OutputDir "rss-reader-webview-debug-log-$timeStamp.txt"
$focusLogPath = Join-Path $OutputDir "rss-reader-webview-focus-log-$timeStamp.txt"
$markerPath = Join-Path $OutputDir "rss-reader-webview-log-marker.txt"
...
switch ($Mode) {
    "prepare" {
        ...
        Write-Host ""
        Write-Host "Vorbereitung fuer WebView-Test abgeschlossen."
        Write-Host "Jetzt die App starten und die betroffenen Reader-/WebView-Aktionen ausfuehren."
    }
    ...
    "collect" {
        ...
        Write-Host "Erzeuge fokussierten WebView-Auszug nach $focusLogPath"
        @(
            "===== WebView fokussierter Auszug ====="
            ""
            "---- App-Debuglog: Reader-/WebView-/Swipe-Hinweise ----"
        ) | Set-Content -Path $focusLogPath -Encoding UTF8

        if (Test-Path $debugLogPath) {
            Select-String -Path $debugLogPath -Pattern "ArticleReaderScreen|WebView|Reader|Artikel wird geladen|Artikel geladen|Swipe|Browser|HTTP-Fehler|Legacy WebView Fehler|about:blank|Seite geladen|Inhalt geladen" -CaseSensitive:$false |
        ...
        @(
            ""
            "---- logcat: Chromium-/WebView-/Render-/Crash-Hinweise ----"
        ) | Add-Content -Path $focusLogPath -Encoding UTF8

        if (Test-Path $crashLogPath) {
            Select-String -Path $crashLogPath -Pattern "de.lolo.rssreader|chromium|WebView|cr_WebView|AwContents|SandboxedProcess|tile memory|AndroidRuntime|FATAL EXCEPTION|HTTP|Renderer|render|HWUI|about:blank|DocumentsUI|ActivityTaskManager" -CaseSensitive:$false |
                ForEach-Object { $_.Line } |
                Add-Content -Path $focusLogPath -Encoding UTF8
        }

        Write-Host ""
        Write-Host "WebView-Logs exportiert:"
